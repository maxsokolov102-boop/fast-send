...
function handleMessage(ws, msg) {
  switch (msg.type) {
    ...
    case 'set-password': {
      const session = ws.session;
      if (!session || ws.role !== 'host') return;
      const pwd = String(msg.password || '').trim();
      if (pwd.length < 4 || pwd.length > 32) {
        return safeSend(ws, {
          type: 'error',
          code: 'bad_password_format',
          message: 'Пароль должен быть от 4 до 32 символов',
        });
      }
      session.password = pwd;
      safeSend(ws, { type: 'password-updated', password: pwd });
      break;
    }
